  ...
  async saveOrder(order: any) {
    try {
      ...
      try {
        ...
        const backendUrl = (import.meta as any)?.env?.VITE_BACKEND_URL ? `${(import.meta as any).env.VITE_BACKEND_URL}/api/orders` : `${window.location.protocol}//${window.location.hostname}:3002/api/orders`;
